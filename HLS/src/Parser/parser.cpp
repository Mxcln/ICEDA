...
int parser::parse_function(std::string &line)
{
    ...
    while ((tok = strtok(NULL, sep)) != NULL)
    {
        ...
        if (key == "int" && cnt < 2)
        {
            _func->set_ret_type(RET_TYPE::RET_INT);
        }
        else if (key == "void")
        {
            _func->set_ret_type(RET_TYPE::RET_VOID);
        }
        else if (cnt == 2)
        {
            _func->set_name(key);
